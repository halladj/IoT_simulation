...
# -*- coding: utf-8 -*-
"""
NS-3 Discovery and Collaboration Protocol Simulation for Mobile IoT Security Research

This simulation implements a two-phase protocol for mobile IoT networks:

Phase 1 - Discovery (UDP, 2-22s):
    Agents broadcast UDP messages to discover neighbors within 80m range
    Port: 8000, Interval: 2s

Phase 2 - Collaboration (TCP, 25-95s):
    Discovered agents establish peer-to-peer TCP connections
    Ports: 9000+, Continuous data exchange

Features:
    - Network flow metrics (duration, packets, bytes, delay, jitter, throughput)
    - Mobility-specific metrics (node density - novel contribution)
    - Dual-protocol dataset (UDP + TCP)
    - RandomWalk2d mobility model for mobile nodes
    - WiFi 802.11n network

Output:
    - simulation_features.csv: Dataset with 16 feature columns
    - discovery-collab-animation.xml: NetAnim visualization
